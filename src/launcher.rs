 ...
 * hundred kilobytes instead of carrying its own copy of the registry
...
/*
 * Cargo installs every binary of this package into the same directory,
 * so prefer the sibling executable and only fall back to PATH.
 */
...
fn run(command: &mut Command) -> io::Result<ExitCode> {
    ...
    /*
     * Replace this process so signals and the exit status reach the
     * caller without a launcher sitting in between.
     *
